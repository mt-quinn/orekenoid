 ...
 *   `E` bounder     a Bounder stands here, placed at map creation and never replaced
...
const LEGEND: Record<string, Paint> = {
  ...
  // Open floor. The Bounder is placed from `LANDING_SPAWNS`, not painted into the cell.
  E: { solid: false },
};
...
/**
 * Where the Landing's Bounders stand, read off the map's `E` glyphs.
 *
 * Authored rather than rolled for, and finite: each one fires once and is gone for the rest of the
 * expedition. The first is on the island in view of the ledge the Seal opens onto, so the player watches
 * it walk and coil before it has cost them anything; the other two occupy the Gallery floor, so the room
 * past the door is inhabited rather than staged.
 */
export const LANDING_SPAWNS: ReadonlyArray<{ x: number; y: number }> = cellsOf("E")
  .map((cell) => ({ x: cell.x + 0.5, y: cell.y + 0.34 }));
